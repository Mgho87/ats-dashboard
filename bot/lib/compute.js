...
function analyze(parsed, filter) {
  ...
  // Daily Leads page (additive, read-only): every in-range record (incl. cancelled, so cancelled
  // leads still appear, marked) with phone + notes. Does NOT change any KPI/revenue/reconciliation.
  const sherryRecords = recs.slice().sort(sortByDateDesc).map(r => ({
    ref: r.ref, date: r.dateKey, client: r.client, phone: r.phone, service: r.service, amount: round2(r.amount),
    status: r.payment, fileStatus: r.delivery, method: r.method, lead: leadLabel(r.lead), notes: r.notes,
  }));
  ...
  return {
    ...
    recentTransactions, sherryRecords, outstandingOrders, googleAdsLeads, pendingDeliveries, topClients,

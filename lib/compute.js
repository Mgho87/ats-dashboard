...
function parseAll(transTable, expTable, settingsTable, now) {
  ...
  for (let i = 0; i < tRows.length; i++) {
    ...
    const lead = (col.lead >= 0 ? String(row[col.lead] == null ? '' : row[col.lead]).trim() : '') || 'Direct';
    ...
    if (allowedPay.size && rawPay && !allowedPay.has(norm(rawPay))) { brokenStatus++; audit.push(ev('warning', 'Unknown payment status', `Row ${sheetRow}: "${rawPay}" not in Settings list`)); }
    ...
    records.push({
      ...
      method, lead, notes, isGoogle: isGoogleLead(lead),
    });
  }
  ...
  return {
    now, today, col, settings, records, expenses,
...
function analyze(parsed, filter) {
  ...
  const inRange = r => r.dateKey >= from && r.dateKey <= to
    && (!svc || r.service === svc) && (!lead || r.lead === lead);
  ...
  for (const r of recs) {
    ...
    bumpRO(leadMap, r.lead, r.amount);
    methodMap[r.method] = (methodMap[r.method] || 0) + r.amount;
    deliveryMap[r.delivery] = (deliveryMap[r.delivery] || 0) + 1;

    if (r.client) {
      const c = clientMap[r.client] || (clientMap[r.client] = { client: r.client, revenue: 0, orders: 0, paid: 0, outstanding: 0, lastDate: '', lead: r.lead, phone: r.phone });
      c.revenue += r.amount; c.orders++; c.paid += paidPart; c.outstanding += outPart;
      if (r.dateKey > c.lastDate) { c.lastDate = r.dateKey; c.lead = r.lead; }
    }
  ...
  const paymentMethods = Object.keys(methodMap).map(n => ({ name: n, value: round2(methodMap[n]) })).sort((a, b) => b.value - a.value);
  ...
  const topClients = Object.values(clientMap).map(c => ({
    ...
    outstanding: round2(c.outstanding), lastDate: c.lastDate, lead: c.lead, phone: c.phone,
  ...
  const compare = buildCompare(parsed, from, to, svc, lead);
  ...
  return {
    ...
    revenueTrend, dailyTrend, topServices, leadSources, paymentSummary, paymentMethods, deliveryBreakdown,
...
function windowAgg(parsed, from, to, svc, lead) {
...
function buildCompare(parsed, from, to, svc, lead) {
  ...
  const cur = windowAgg(parsed, from, to, svc, lead);
  const prev = windowAgg(parsed, pFrom, pTo, svc, lead);

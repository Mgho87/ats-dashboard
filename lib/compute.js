...
function analyze(parsed, filter) {
  ...
  const sherryRecords = recs.slice().sort(sortByDateDesc).map(r => ({
    date: r.dateKey, client: r.client, phone: r.phone, service: r.service, amount: round2(r.amount),
    status: r.payment, fileStatus: r.delivery, lead: leadLabel(r.lead), notes: r.notes,
  }));

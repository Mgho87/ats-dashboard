...
function parseAll(transTable, expTable, settingsTable, now) {
  ...
  for (let i = 0; i < tRows.length; i++) {
    ...
    const lead = (col.lead >= 0 ? String(row[col.lead] == null ? '' : row[col.lead]).trim() : '') || 'Other';
    ...
    flagUnknown('leads', allowedLead, col.lead >= 0 ? row[col.lead] : ''); flagUnknown('services', allowedSvc, col.service >= 0 ? row[col.service] : ''); flagUnknown('methods', allowedMethod, col.method >= 0 ? row[col.method] : '');
    ...
    records.push({
      ...
      method, lead, notes, isGoogle: isGoogleLead(lead), isReturning: /return/i.test(lead),
    });
  }
...
function analyze(parsed, filter) {
  ...
  for (const r of recs) {
    ...
    if (r.client) {
      const c = clientMap[r.client] || (clientMap[r.client] = { client: r.client, revenue: 0, orders: 0, paid: 0, outstanding: 0, lastDate: '', firstDate: r.dateKey, lead: r.lead, acqLead: r.lead, phone: r.phone, months: {}, invoices: [] });
      ...
      if (r.dateKey < c.firstDate) { c.firstDate = r.dateKey; c.acqLead = r.lead; }
